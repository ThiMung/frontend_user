...
import toast, { Toaster } from 'react-hot-toast'; // Thư viện hiển thị thông báo (Alert) dạng Toast trượt
...
const LoginPage = () => { 
  ...
  // State quản lý mảng lỗi Validation trả về từ Form Request của Laravel
  const [errors, setErrors] = useState({}); 
  // State quản lý trạng thái xoay vòng loading của nút bấm khi đang đợi API phản hồi 
  const [isLoading, setIsLoading] = useState(false); 
 
  // Hàm xử lý kích hoạt khi người dùng nhấn nút bấm LogIn
  const handleSubmit = async (e) => { 
    ...
    try { 
      // Đính kèm trường 'required_role' vào body gửi lên để thỏa mãn điều kiện validate của Laravel
      const requestData = { 
        ...formData, 
      ...
      // Bóc tách dữ liệu linh hoạt, phòng trường hợp backend bọc dữ liệu trong nhánh data lồng nhau
      ...
      // Nhánh 1: Bắt lỗi Validation Form của Laravel FormRequest
      ...
      // Nhánh 3: Xử lý các lỗi hệ thống ngầm, lỗi kết nối hoặc sập cơ sở dữ liệu
      else { 
        toast.error(err.response?.data?.message || 'Server connection error. Please try again later.', { position: 'top-right' }); 
      } 
    } finally { 
      setIsLoading(false); // Tắt hiệu ứng loading sau khi xử lý xong
    } 
  }; 
